...
def extract_tables_from_holding(file_path):
    ...
    # Read the full sheet into a DataFrame
    df = pd.read_excel(file_path, sheet_name=0, header=None)

    # Extracting First table
    start_row_1 = df[df.iloc[:, 0] == 'Client Equity Code/UCID/Name'].index
    if start_row_1.empty:
        raise ValueError("Could not find the start of the first table.")
    start_row_1 = start_row_1[0]
    ...
    second_table_start = df[df.iloc[:, 0] == 'Instrument Name'].index
    if second_table_start.empty:
        raise ValueError("Could not find the header row for the second table.")
    second_table_start = second_table_start[0]

    # Read the second table starting from the identified header row
    second_table = pd.read_excel(file_path, sheet_name=0, header=second_table_start)

    # Find the row where "Instrument Name" contains "Total"
    total_row_indices = second_table[second_table['Instrument Name'].astype(str).str.contains('Total', na=False)].index

    if not total_row_indices.empty:
        # If "Total" is found, slice the table up to and including that row
        total_row = total_row_indices[0]
        second_table = second_table.iloc[:total_row + 1]
    ...
    second_table = second_table[[col for col in required_columns if col in second_table.columns]]
